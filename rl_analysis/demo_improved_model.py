#!/usr/bin/env python3
"""
Демонстрация улучшенной модели с исправлениями
Использует NetworkEchoEnvImproved для совместимости
"""
...
def demo_improved_model(model_path, episodes=3, interactive=False):
    """Демонстрирует улучшенную модель"""
    print(f"🤖 ДЕМОНСТРАЦИЯ УЛУЧШЕННОЙ МОДЕЛИ")
    print("=" * 50)
    print(f"📥 Загрузка модели: {model_path}")
    
    # Создаем окружение
    env = NetworkEchoEnvImproved(log_actions=True, log_path="demo_improved_actions.jsonl")
    ...
    for episode in range(episodes):
        ...
        # Таймаут для предотвращения вечного цикла
        max_steps = 2000
        ...
        while not (done or truncated) and step_count < max_steps:
            ...
            if time.time() - start_time > 300:  # 5 минут таймаут
                print("⚠️ Таймаут эпизода (5 минут)")
                break
...
def create_demo_visualization(rewards, lengths, actions):
    ...
    plt.tight_layout()
    plt.savefig('demo_improved_results.png', dpi=300, bbox_inches='tight')
    print("📊 Визуализация сохранена: demo_improved_results.png")
    plt.show()

def main():
    """Главная функция"""
    print("🤖 ДЕМОНСТРАЦИЯ УЛУЧШЕННОЙ МОДЕЛИ")
    print("=" * 50)
    
    # Находим доступные модели
    model_files = []
    for file in os.listdir('.'):
        if file.endswith('.zip') and ('improved' in file or 'light' in file):
            model_files.append(file)
    
    if not model_files:
        print("❌ Не найдены улучшенные модели (.zip файлы с 'improved' или 'light' в названии)")
        return
    ...
    try:
        ...
        if mode == 1:
            demo_improved_model(model_path, episodes=3, interactive=False)
        elif mode == 2:
            demo_improved_model(model_path, episodes=1, interactive=True)
        elif mode == 3:
            print("👋 До свидания!")
        else:
            print("❌ Неверный выбор")
