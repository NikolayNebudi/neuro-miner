...
Добавлены бонусы за победу, штрафы за бездействие и уменьшена максимальная длина эпизода
"""
...
class NetworkEchoEnvEnhanced(gym.Env):
    metadata = {"render_modes": ["human"], "render_fps": 4}

    def __init__(self, config=None, log_actions=False, log_path="enhanced_actions_log.jsonl", max_log_entries=10000):
        super().__init__()
        self.proc = None
        ...
        self._step_count = 0
        self._max_steps = 500  # Уменьшено с 2000 до 500 для более частого завершения
        self._last_action = None
        self._consecutive_wait_actions = 0  # Счетчик подряд идущих действий wait
    ...
    def _setup_logging(self):
        ...
        try:
            ...
            self.log_path = f"enhanced_log_{timestamp}.jsonl"
    ...
    def _calculate_enhanced_reward(self, result, base_reward, action_name):
        """Вычисляет улучшенную награду с бонусами за победу и штрафами за бездействие"""
        ...
        if node_change > 0:
            enhanced_reward += node_change * 100  # Увеличен бонус за захват узлов
        elif node_change < 0:
            enhanced_reward += node_change * 50  # Увеличен штраф за потерю узлов
        
        # Бонусы за ресурсы
        if dp_change > 0:
            enhanced_reward += dp_change * 0.2  # Увеличен бонус за DP
        if cpu_change > 0:
            enhanced_reward += cpu_change * 0.5  # Увеличен бонус за CPU
        
        # Штраф за увеличение trace level
        if trace_change > 0:
            enhanced_reward -= trace_change * 5  # Увеличен штраф за trace level
        
        # Бонусы за действия
        log_events = result.get('logEvents', [])
        for event in log_events:
            if event.get('type') == 'capture_complete':
                enhanced_reward += 200  # Увеличен бонус за завершение захвата
            elif event.get('type') == 'upgrade_hub':
                enhanced_reward += 500  # Увеличен бонус за улучшение хаба
            elif event.get('type') == 'build':
                enhanced_reward += 50   # Увеличен бонус за строительство
            elif event.get('type') == 'enemy_killed':
                enhanced_reward += 30   # Увеличен бонус за убийство врага
        
        # Штрафы за бездействие
        if action_name == "wait":
            self._consecutive_wait_actions += 1
            if self._consecutive_wait_actions > 3:
                enhanced_reward -= 5 * self._consecutive_wait_actions  # Штраф за длительное бездействие
        else:
            self._consecutive_wait_actions = 0  # Сбрасываем счетчик
        
        # Бонусы за активные действия
        if action_name == "capture":
            enhanced_reward += 10  # Бонус за попытку захвата
        elif action_name == "build":
            enhanced_reward += 5   # Бонус за строительство
        elif action_name == "upgrade_hub":
            enhanced_reward += 15  # Бонус за улучшение
        
        # Бонус за победу
        if result.get('win', False):
            enhanced_reward += 1000  # Большой бонус за победу
        
        # Штраф за поражение (высокий trace level)
        if current_trace_level >= 300:
            enhanced_reward -= 500  # Большой штраф за поражение
        
        # Небольшой бонус за выживание (уменьшен)
        if not result.get('done', False):
            enhanced_reward += 0.5  # Уменьшен бонус за каждый шаг
    ...
    def _log_action_enhanced(self, episode, step, action, reward, enhanced_reward, state_summary):
    ...
    def step(self, action):
        ...
        enhanced_reward = self._calculate_enhanced_reward(result, base_reward, action_name)
        
        self._total_reward += enhanced_reward
        
        # Логируем действие
        state_summary = self._extract_state_summary(self._state)
        self._log_action_enhanced(self._episode, self._step_count, action_name, base_reward, enhanced_reward, state_summary)
        ...
        return self._get_observation(), enhanced_reward, done, False, {}
