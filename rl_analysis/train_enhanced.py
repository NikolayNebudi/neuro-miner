#!/usr/bin/env python3
"""
Скрипт обучения с улучшенным окружением
Включает бонусы за победу, штрафы за бездействие и уменьшенную длину эпизодов
"""
...
def create_env(log_actions=True):
    ...
    def make_env():
        env = NetworkEchoEnvEnhanced(
            ...
            log_path="enhanced_training_log.jsonl",
...
def train_enhanced_model(total_timesteps=1000000, save_interval=50000):
    """Обучает модель с улучшенными наградами"""
    print("[DEBUG] Вход в train_enhanced_model")
    ...
    try:
        model = PPO(
            ...
            max_grad_norm=0.5,
            target_kl=None,
            tensorboard_log="./tensorboard_logs_enhanced/",
            verbose=0
        )
    ...
    os.makedirs("./checkpoints_enhanced/", exist_ok=True)
    os.makedirs("./tensorboard_logs_enhanced/", exist_ok=True)
    ...
    print(f"  🔄 N epochs: 4")
    print(f"  ⏱️ Максимальная длина эпизода: 500")
    print(f"  🏆 Бонус за победу: 1000")
    print(f"  ⚠️ Штраф за бездействие: -5 за шаг после 3 wait")
    print()
    ...
    try:
        ...
        final_model_path = f"enhanced_final_model_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
        ...
        interrupted_model_path = f"enhanced_interrupted_model_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
        ...
        error_model_path = f"enhanced_error_model_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
    ...
    print("[DEBUG] Выход из train_enhanced_model")
    return model

def analyze_enhanced_training():
        ...
        for line in f:
            try:
                ...
                if data.get('type') == 'action':
                    ...
                    rewards.append(data.get('enhanced_reward', 0))
    ...
    create_enhanced_visualization(episodes, actions, rewards)

def create_enhanced_visualization(episodes, actions, rewards):
    ...
    print("📊 Визуализация сохранена: enhanced_training_analysis.png")
    plt.show()
...
def main():
    ...
    try:
        ...
        if choice == 1:
            ...
            model = train_enhanced_model(total_steps, save_interval)
            
        elif choice == 2:
            analyze_enhanced_training()
